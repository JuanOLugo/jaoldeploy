...
export const CreateProduct = async (
  ...
  const codigoExiste = await ProductoModel.findOne({
    codigoBarra: codigo,
    usuariocontenedor: _id,
  });

  if (codigoExiste)
    return res.status(400).send({ error: "El codigo ya existe" });
...
export const DeleteProduct = async (
  ...
  try {
    ...
    if(!misProductos) return res.status(404).send({ msg: "Producto no encontrado" });
...
export const UpdateProduct = async (
  ...
  try {
    const productUpdate = await ProductoModel.findOneAndUpdate({_id: _id, usuariocontenedor: UserId}, {
      nombre,
      precioDeCosto,
      precioDeVenta,
      proveedorNombre,
    ...
    if(!productUpdate) return res.status(404).send({ msg: "Producto no encontrado" });
